function sepackage_queue(startDate, endDate, series) {
	var sort1 = 
	{ 
	...
	query = "+PATH:'/app:company_home/st:sites/cm:controlled/cm:documentLibrary//*' AND +TYPE:'analog:controlled' AND {http://www.analog.com/model/content/1.0}adi_series:" + series + " AND cm:modified:[" + startDate + " TO " + endDate + "]";
...
var DMRFolder = reportFolder.childByNamePath("Daily Modification Series Reports");
if (DMRFolder === null && reportFolder.hasPermission("CreateChildren"))
{
   // create the folder for the first time
   DMRFolder = reportFolder.createFolder("Daily Modification Series Reports");
}
var series = DMRFolder.properties['cm:title'];

//print ("Start: " + startDate + " End: " + endDate + " Series: " + series);
var results = sepackage_queue(startDate, endDate, series);
...
if (DMRFolder !== null && DMRFolder.hasPermission("CreateChildren"))
{
   ...
   var reportName = "daily modification series report-" + series + "-" + filedate + ".csv";
   var logFile = DMRFolder.childByNamePath(reportName);
   if (logFile === null)
   {
      logFile = DMRFolder.createFile(reportName);
